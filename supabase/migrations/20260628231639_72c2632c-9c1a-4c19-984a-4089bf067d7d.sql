...
declare
  ...
  v_aplicados_por_mes jsonb := '{}'::jsonb;
  v_lancados_por_mes jsonb := '{}'::jsonb;

  v_distribuicao jsonb;
  v_mes_fmt text;
  v_partes text[];
  v_mes_lancamento text;
begin
  ...
  if v_n_meses > 0 then
    ...
    for v_mes_idx in 1..v_n_meses loop
      v_aplicados_por_mes := jsonb_set(v_aplicados_por_mes, array[v_mes_keys[v_mes_idx]], '[]'::jsonb, true);
      v_lancados_por_mes := jsonb_set(v_lancados_por_mes, array[v_mes_keys[v_mes_idx]], '[]'::jsonb, true);
    end loop;
  end if;
  ...
  if v_n_meses > 0 then
    ...
    loop
      ...
      -- A) Aplicados: adicionar em cada mês alcançado
      if jsonb_array_length(v_distribuicao) > 0 then
        ...
        begin
          for j in 0..(jsonb_array_length(v_distribuicao) - 1) loop
            ...
            v_lista := v_aplicados_por_mes->v_mes_key;
            v_lista := v_lista || v_detalhe;
            v_aplicados_por_mes := jsonb_set(v_aplicados_por_mes, array[v_mes_key], v_lista, true);
          end loop;
        end;
      end if;

      -- B) Lançados: adicionar no mês de criação do abatimento (se estiver no calendário)
      v_mes_lancamento := to_char(v_abat.criado_em at time zone 'America/Sao_Paulo', 'YYYY-MM');
      if v_lancados_por_mes ? v_mes_lancamento then
        declare
          v_detalhe_l jsonb;
          v_lista_l jsonb;
        begin
          v_detalhe_l := jsonb_build_object(
            'abatimento_id', v_abat.id,
            'data_lancamento', v_abat.criado_em,
            'data_lancamento_brasil', to_char(v_abat.criado_em at time zone 'America/Sao_Paulo', 'DD/MM/YYYY'),
            'hora_lancamento_brasil', to_char(v_abat.criado_em at time zone 'America/Sao_Paulo', 'HH24:MI'),
          ...
          v_lista_l := v_lancados_por_mes->v_mes_lancamento;
          v_lista_l := v_lista_l || v_detalhe_l;
          v_lancados_por_mes := jsonb_set(v_lancados_por_mes, array[v_mes_lancamento], v_lista_l, true);
        end;
      end if;
    end loop;
  end if;
    ...
    'meses', coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'abatimentos_detalhados', coalesce(v_aplicados_por_mes->c.mes, '[]'::jsonb),
          'abatimentos_aplicados_no_mes', coalesce(v_aplicados_por_mes->c.mes, '[]'::jsonb),
          'abatimentos_lancados_no_mes', coalesce(v_lancados_por_mes->c.mes, '[]'::jsonb)
        )
        order by c.mes
      ),
      '[]'::jsonb
