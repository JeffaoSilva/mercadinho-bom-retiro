...
--
-- Esta revisão corrige regressões:
--  * ordem obrigatória: validação completa ANTES da idempotência
--  * UUID inválido tratado apenas via invalid_text_representation
--  * itens duplicados => ITEM_DUPLICADO (sem agregação)
--  * advisory lock com namespace 'reserva_checkout_pix:'
--  * comparação idempotente completa e bidirecional (EXCEPT ALL)
--  * estados encerrados => IDEMPOTENCIA_ENCERRADA
--  * cliente/tablet validados somente por existência
--  * prateleira com três códigos distintos
--  * preço-base validado; promoção por 'tipo'; PROMOCAO_INVALIDA
--  * contrato de retorno com ok / codigo / reutilizada
--  * validação numérica por valor numeric (sem regex) preservada
-- =====================================================================
...
BEGIN
  ...
  ---------------------------------------------------------------------
  -- 2. chave_idempotencia (uuid textual)
  ---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------
  -- 3. mercadinho_id (obrigatório, bigint)
  ---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------
  -- 4. cliente_id (opcional; NULL = visitante)
  ---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------
  -- 7. Validação completa de TODOS os itens (antes da idempotência)
  ---------------------------------------------------------------------
  FOR v_item IN SELECT value FROM pg_catalog.jsonb_array_elements(v_itens) LOOP
    ...
    -- item duplicado (sem agregação automática)
    IF EXISTS (
      SELECT 1
  ...
  ---------------------------------------------------------------------
  -- 8. Idempotência: advisory lock transacional com namespace
  ---------------------------------------------------------------------
  ...
  IF FOUND THEN
    -- cabeçalho: mercadinho, cliente, tablet
    ...
    -- itens: comparação semântica bidirecional
    SELECT EXISTS (
  ...
  ---------------------------------------------------------------------
  -- 9. Existência: mercadinho, cliente, tablet
  ---------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------
  -- 10. Locks (id ASC), prateleira, preço, promoção, estoque, total
  ---------------------------------------------------------------------
  ...
  LOOP
    -- lock pela identidade da prateleira
    ...
    -- promoção vigente por tipo: 'produto' tem prioridade sobre 'global'
    v_desconto := NULL;
